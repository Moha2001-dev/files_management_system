...
class EditFile extends StatefulWidget {
  EditFile({super.key, required this.file});
  FMSFile file;
  @override
...
class _editFileState extends State<EditFile> {
  ...
  void initState() {
    ...
    initPlatformState();
    getSettings();
    _fileTitle = widget.file.title!;

    _filePath = widget.file.path;
    _fileNumber = widget.file.fileNum;
    if(widget.file.date != null && (widget.file.date??"").isNotEmpty){
      _fileDate = widget.file.date!;
      DateTime date = DateTime.parse(_fileDate);
      selectedDate.hDay = date.day;
      selectedDate.hMonth = date.month;
      selectedDate.hYear = date.year;
      dateController.text = selectedDate.toString();
      _fileDate = selectedDate.toString();
    }
    ...
    nameController.text = _fileTitle;
    numberController.text = _fileNumber;
  }
                                    ...
                                    child: IconButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        if (pathValidate && numValidate && dateValidate) {
                                          ...
                                          String pathPlus = _filePath;
                                          if(widget.file.path != _filePath){
                                            //copy file
                                            ...
                                            await File(widget.file.path).delete();
                                            File newFile = await file.copy(join(appDucs.path,'$_fileNumber${extension(file.path)}'));
                                            pathPlus = newFile.path;
                                          }

                                          int isInsert =await insertToDatabase(pathPlus);
                                          if(isInsert != 0){
                                            ScaffoldMessenger.of(context).showSnackBar(
 ...
 */
  ...
  insertToDatabase(String path) async{
    ...
    print(path);
    int result = await sql.updateData("UPDATE 'FilesInfo' SET filenumber = '$_fileNumber' , title = '${_fileTitle.isEmpty? 'empty' : _fileTitle}' , date = '${(_fileDate?? "").isEmpty? "1010-01-01" : fileDate}', filepath='$path' WHERE filenumber = '${widget.file.fileNum}';");
    return result;
  }
